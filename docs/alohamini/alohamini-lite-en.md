...
Find the `_ROBOT_SPECS` dict (around line 112). Locate `"alohamini2pro"` and insert a new `"alohamini2lite"` entry **after** it (add a comma after the closing `}`, then insert):
...
Find the comment above `robot_model: str = "alohamini1"`. Append a line after the `alohamini2pro` line:

```
    # alohamini2lite– am-follower-6dof-lite (all STS3215 6-DoF), base sts3215, lift sts3215, lead=84 mm/rev
```
...
For other models, replace `--robot_model` with: `alohamini1`, `alohamini2`, or `alohamini2pro`.
